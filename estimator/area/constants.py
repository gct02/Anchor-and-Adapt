...
NODE_FEATURES = {
    "mem_buf": [
        *[f"anchor_{m}" for m in MEM_AREA_METRICS],
        "bitwidth", "array_size", 
        "is_local", "is_global", "is_port", "is_interface", 
        "is_ram", "is_rom", "2p_read", "2p_write",
        "max_reads_in_iter", "max_writes_in_iter",
        "part_factor", "part_size", "part_type",
        "part_bitsize", "theoretical_bram"
    ],
    "op": [
        *OPTYPES,
    ...
    "region": [
        *[f"anchor_{m}" for m in AREA_METRICS],
        "is_top_function", "region_latency", "trip_count", 
        "resmii", "recmii", "is_top_loop", "is_innermost", 
        "num_phi_nodes", "is_part_of_perfect_nest", 
        "unroll_factor", "pipeline", "pipeline_off", 
        "pipelined_parent", "loop_flatten", "loop_merge"
    ]
}
...
FEATURES_TO_LOG_SCALE = {
    *{f"anchor_{m}" for m in AREA_METRICS},
    ...
    "max_reads_in_iter", "max_writes_in_iter",
    "unroll_factor", "part_factor", "part_size",
    "part_bitsize", "theoretical_bram"
}
