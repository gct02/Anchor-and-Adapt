...
from estimator.area.constants import AREA_METRICS
...
def evaluate(
    ...
    for i, m in enumerate(AREA_METRICS):
        preds_dict[m] = preds[:, i].view(-1, 1)
        if m in ["bram", "dsp"]:
            preds_dict[m][preds_dict[m] <= 0.5] = 0.0
        else:
            preds_dict[m] = preds_dict[m].relu()
...
def evaluate_ensemble(
    ...
    all_preds_dict = {m: [] for m in AREA_METRICS}
...
def export_eval_results(test_bench, eval_rpt_dict, output_dir):
    ...
    for m, rpt in eval_rpt_dict.items():
        ...
        plot_prediction_bars(
            ...
            errors=rpt.errors, mape=rpt.smape
        )
        plot_prediction_scatter(
...
if __name__ == "__main__":
    ...
    def parse_arguments() -> Dict[str, Any]:
        parser = argparse.ArgumentParser(
            description='Pre-train estimator to infer the Area (BRAMs, DSPs, FFs, and LUTs) of HLS designs.'
        )
        ...
        parser.add_argument('-d', '--dataset-dir', type=str, default='estimator/area/dataset',
                            help='Dataset root directory. Default: \'estimator/area/dataset\'.')
    ...
    train_loader, test_loader = prepare_train_test_loaders(
        AreaDataset, dataset_dir, test_bench, 
