...
from estimator.cycles.constants import (
    NODE_DIM_DICT,
    NODE_TYPES, 
    EDGE_TYPES,
    MACRO_NODE_TYPES, 
...
class ClockCyclesModel(nn.Module):
    def __init__(
        self,
        hidden_dim: int,
        ...
        self.jk_micro = HeteroJumpingKnowledge(self.macro_node_types, mode='max')
        ...
        self.jk_macro = JumpingKnowledge(mode='max')
    ...
    def forward(
        ...
        emb_max = global_max_pool(x, region_batch, batch_size)
        emb = torch.cat([emb_att, emb_max, y_anchor], dim=-1)
