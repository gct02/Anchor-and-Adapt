...
from estimator.cycles.train import export_eval_results, evaluate_ensemble, METRIC_NAME
...
def main(args: Dict[str, str]):
    ...
    train_dataset_dir = args.get("train_dataset_dir", "estimator/cycles/dataset")
    ...
    dataset = ClockCyclesDataset()

    anchor = dataset.preprocess_data(
        graph=graph,
        target={METRIC_NAME: graph.anchor_metrics[METRIC_NAME]},
        solution=0
    )
    ...
    for path in model_paths:
        ...
        for n, p in model.named_parameters():
            if n.startswith('mlp.') and not n.startswith('mlp.0.'):
                p.requires_grad  = True
                trainable_params.append(p)
            else:
                p.requires_grad = False

        optimizer = torch.optim.AdamW(trainable_params, lr=5e-4, weight_decay=0.0)
    ...
    export_eval_results(graph.benchmark, final_rpt, output_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--train-dataset-dir", type=str, default="estimator/cycles/dataset",
