...
from estimator.mtl.dataset import MultiTaskDataset
from estimator.mtl.constants import EXTENDED_METRICS
...
def run_inference(
    ...
    for data in loader:
        ...
        pred = torch.expm1(out + data.y_anchor.view_as(out))
...
def evaluate(
    ...
    errors_dict, smape_dict = {}, {}

    for i, m in enumerate(EXTENDED_METRICS):
        preds_dict[m] = preds[:, i].view(-1, 1)
        targets_dict[m] = targets[:, i].view(-1, 1)
        errors_dict[m] = smape_loss(
            preds_dict[m], targets_dict[m], reduce=False
        ).view_as(preds_dict[m])
        smape_dict[m] = torch.mean(errors_dict[m], dim=0).item()

    if not as_tensor:
        for m in EXTENDED_METRICS:
            preds_dict[m] = preds_dict[m].flatten().tolist()
            targets_dict[m] = targets_dict[m].flatten().tolist()
            errors_dict[m] = errors_dict[m].flatten().tolist()
    ...
    return {
        m: EvalReport(
            indices=indices,
            preds=preds_dict[m],
            targets=targets_dict[m],
            errors=errors_dict[m],
            smape=smape_dict[m]
        )
        for m in EXTENDED_METRICS
    }
...
def evaluate_ensemble(
    ...
    all_preds_dict = {m: [] for m in EXTENDED_METRICS}
    for path in snapshot_paths:
        ...
        preds, targets = run_inference(model, loader)
        for i, m in enumerate(EXTENDED_METRICS):
            all_preds_dict[m].append(preds[:, i].view(-1, 1))
    ...
    for i, m in enumerate(EXTENDED_METRICS):
        targets_dict[m] = targets[:, i].view(-1, 1)
        preds_dict[m] = torch.stack(all_preds_dict[m]).mean(dim=0).view_as(targets_dict[m])
        errors_dict[m] = smape_loss(
            preds_dict[m], targets_dict[m], reduce=False
        ).view_as(preds_dict[m])
        smape_dict[m] = torch.mean(errors_dict[m], dim=0).item()
...
def train_and_eval(
    ...
    final_rpt_dict = evaluate_ensemble(model, test_loader, snap_paths)
    print("Final Ensemble SMAPE (Test Set):")
    for m, rpt in final_rpt_dict.items():
        print(f"  {m}: {rpt.smape:.4f}")

    export_eval_results(test_bench, final_rpt_dict, output_dir)


def export_eval_results(test_bench, eval_rpt_dict, output_dir):
    output_dir = Path(output_dir)
    for m, rpt in eval_rpt_dict.items():
        barplot_path = output_dir / f'{m}_barplot.png'
        scatterplot_path = output_dir / f'{m}_scatterplot.png'
        rpt_path = output_dir / f'{m}_report.csv'
        plot_prediction_bars(
            rpt.targets, rpt.preds, rpt.indices, test_bench, m, barplot_path, 
            errors=rpt.errors, mape=rpt.mape
        )
        plot_prediction_scatter(
            rpt.targets, rpt.preds, test_bench, m, scatterplot_path, 
            mape=rpt.mape
        )
        export_predictions_as_csv(
            rpt.targets, rpt.preds, rpt.errors, rpt.indices, rpt_path
...
if __name__ == "__main__":
    ...
    def parse_arguments() -> Dict[str, Any]:
        parser = argparse.ArgumentParser(
            description='Pre-train estimator to infer multiple QoR metrics of HLS designs.'
        )
        parser.add_argument('-t', '--test-bench', type=str, required=True,
                            help='Name of the benchmark to use for testing.')
        parser.add_argument('-d', '--dataset-dir', type=str, default='estimator/mtl/dataset',
                            help='Dataset root directory. Default: \'estimator/mtl/dataset\'.')
    ...
    train_loader, test_loader = prepare_train_test_loaders(
        MultiTaskDataset, dataset_dir, test_bench, 
