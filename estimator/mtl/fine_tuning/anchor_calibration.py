...
from estimator.mtl.train import export_eval_results, evaluate_ensemble, EXTENDED_METRICS
...
def main(args: Dict[str, str]):
    ...
    train_dataset_dir = args.get("train_dataset_dir", "estimator/mtl/dataset")
    ...
    anchor = dataset.preprocess_data(
        graph=graph,
        target={m: float(graph.anchor_metrics[m]) for m in EXTENDED_METRICS},
        solution=0
    )
    ...
    test_loader = prepare_loader(
        dataset_class=MultiTaskDataset,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--train-dataset-dir", type=str, default="estimator/mtl/dataset",
